...
- 언리얼엔진의 리플렉션 시스템 동작 원리에 대해 설명하면? UBT(Unreal Build Tool)이 리플렉션 키워드를 탐색한뒤, UHT(Unreal Header Tool)이 해당 .cpp를 파싱해 리플렉션 데이터를 수집한다 수집된 데이터는 별개의 C++ 코드(generated.h / .cpp)로 저장됩니다 이후 빌드 시 기존 코드에 생성된 C++ 코드(generated.h)를 추가해 컴파일 합니다 ([**참고**](https://hyo-ue4study.tistory.com/182))
...
- CDO 개념에 대해 설명하면? 클래스 기본 객체 ([**참고**](https://dlemrcnd.tistory.com/99)), ([**참고**](https://velog.io/@15ywt/%EC%96%B8%EB%A6%AC%EC%96%BC-CDOClass-Default-Object))

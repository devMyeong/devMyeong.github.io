...
- 서버에서 결정된 게임 상태 변화가 클라이언트의 시각적 효과로 이어질 때, 어떤 데이터를 Replication 해야 하는가? 서버에서는 판정이나 권한이 필요한 로직을 처리하고, 클라이언트가 최종 결과를 재현하는 데 필요한 최소한의 상태 데이터만 Replication하는 것이 좋습니다 예를 들어 서버가 피격 판정 후 구체의 색상을 결정했다면, 전체 처리 과정을 전송하기보다 색상 값만 Replicate하고 클라이언트는 OnRep 등을 통해 색상 변경 및 플래시 같은 시각 효과를 자체적으로 처리할 수 있습니다 ( 19 : 44 )

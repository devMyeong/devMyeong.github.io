...
void game(vector<int>& warp, vector<bool>& visited, int x, int c)
{
...
void getCityCount(vector<vector<int>>& graph, int node, int target_distance)
{
    // 필요한 변수 초기화
    queue<pair<int,int>> que;

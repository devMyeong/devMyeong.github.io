...
bool search(int i, int j, int by, int bx, char alp)
{
	...
	for (int k = 0; k < 4; ++k)
	{
		...
		if (alp != a[ny][nx]) continue;

		ret = ret + search(ny, nx, i, j, alp);
	}
...
{
    vector<vector<int>> tmp(301, vector<int>(301, 0));
...
```cpp
// https://astrid-dm.tistory.com/372 참고

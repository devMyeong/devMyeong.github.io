...
void getMaxNum(int x, int y, int cnt, int curScore)
{
    ...
    for(int i=0; i<4; i++)
    {
        ...
        visited[nx][ny] = 1;
        getMaxNum(nx, ny, cnt+1, curScore + mymap[nx][ny]);
        visited[nx][ny] = 0; // 백트래킹
    }

    if(x-1 >= 1 && y-1 >= 1 && x+1 <= N) //ㅓ
    {
        ans = max(ans, (mymap[x-1][y] + mymap[x][y-1] + mymap[x][y] + mymap[x+1][y]));
    }
    ...
    {
        ans = max(ans, (mymap[x][y] + mymap[x+1][y] + mymap[x+1][y-1] + mymap[x+1][y+1]));
    }
    if(y-1 >= 1 && y+1 <= M && x+1 <= N) //ㅜ
    {
        ans = max(ans, (mymap[x][y-1] + mymap[x][y] + mymap[x][y+1] + mymap[x+1][y]));
    }

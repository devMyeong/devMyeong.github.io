---
title:  "프로그래머스 코딩테스트 고득점 Kit ( 깔끔하고, 쉽고, 시간복잡도가 좋은 코드 )"
...
int solution(int n, vector<vector<int>> wires)
{
    ...
    for (auto wire : wires)
    {
        ...
        answer = min(answer, abs(getLength(wire[0]) - getLength(wire[1])));
...
int solution(string name)
{
    ...
    for(int i = 0; i < name.length(); i++)
    {
        if(name[i] == 'A')
        {
            ...
            // 현재 인덱스까지 왼쪽으로 이동했을때와 오른쪽으로 이동했을때 각각 몇칸 이동해야 하는지 구한다
            int left = i == 0 ? 0 : i - 1;
            int right = name.length() - target;

            // (1) A로만 이루어진 문자열의 왼쪽을 먼저 탐색한 뒤 되돌아가 오른쪽을 탐색할 때가 최소 ex) BBAAAAAAABBBB
            // (2) A로만 이루어진 문자열의 오른쪽을 먼저 탐색한 뒤 되돌아와 왼쪽을 탐색할 때가 최소 ex) BBBBAAAAAAABB
    ...
    // 'Z' - c + 1 에서 +1을 해주는 이유는 A에서 Z로 변환한 뒤, 계산해 주기 위함이다
    for(auto c : name) 
...
{
    // 다섯번이 수행됬을 시점에 sum이 target과 같다면 answer++ 한후 함수 빠져나옴
    ...
    while(!que.empty())
    {
        ...
        for(int i = 0; i < 4; ++i)
        {
            ...
            if(nx >= 0 && ny >= 0 && nx < col && ny < row)
            {
                ...
                {
                    visited[ny][nx] = true;
                    que.push(make_pair(make_pair(ny, nx), cnt + 1));
                }
            }
        }
    }
...
bool dfs(string cur, vector<vector<string>> tickets, vector<string>& answer, vector<bool>& visit, int use_ticket)
{
    answer.push_back(cur);

    // 사용한 티켓 개수가 티켓의 총 개수와 같다면 항공권을 모두 사용했다는 뜻이다
    if (use_ticket == tickets.size()) return true;
    
    // 사용할 수 있는 티켓을 찾아 사용하고, 다음 노드를 방문한다
    for(int i = 0 ; i < tickets.size(); i++)    
    {
        if (tickets[i][0] == cur && visit[i] == false)
        {
            visit[i] = true;
            bool complete = dfs(tickets[i][1], tickets, answer, visit, use_ticket + 1);

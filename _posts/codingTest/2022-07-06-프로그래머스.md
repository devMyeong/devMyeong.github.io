...
int solution(vector<int> citations)
{
    ...
    // 문제 기준으로 6, 5, 3, 1, 0 순으로 정렬된다
    sort(citations.begin(), citations.end());
    for(int i = 0; i < citations.size(); ++i)
    {
        // H_Index 이상 인용된 인덱스 위치를 찾는다
        // 정렬이 되었기 때문에 만약 return이 성립하게 된다면
        // H_Index에 해당하는 인덱스 앞으로는 H_Index번 이상 인용된 논문이 H_Index개 이상 있다는게 보장된다
        // H_Index에 해당하는 인덱스 뒤로는 H_Index 이하로 인용 되었음이 보장된다

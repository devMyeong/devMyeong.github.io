		...
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < n; j++)
			{
				...
				if (!visited[i][j])
				{
					...
					// 스택에 저장된 모든 국가들의 인구수를 현재 연합의 인구수 평균값으로 업데이트함
					while (!que.empty())
					{
						auto pos = que.front();
						que.pop();
	...
	for (int i = 0; i < R; i++)
	{
		for (int j = 0; j < C; j++)
		{
			if (0 < board[i][j]) // 미세먼지 있는 칸
			{
				...
				if (dust == 0) continue; // 문제에서 소수점은 버린다 라고 했다
...
int main()
{
	...
	for (int s = 0; s < n * n; s++)
	{
		...
		int selected_injury = -1, selected_empty = -1, selected_y = -1, selected_x = -1;
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
			{
				...
				int injury = 0, empty = 0;
				for (int dir = 0; dir < 4; dir++)
				{
					int dy = i + y_direction[dir], dx = j + x_direction[dir];
					// 범위를 벗어나는 경우 건너뛰기
				...
				// 조건에 맞는 자리 선택하기
				if (injury > selected_injury)
				{
					selected_injury = injury, selected_empty = empty;
					selected_y = i, selected_x = j;
				}
				else if (injury == selected_injury)
				{
					if (empty > selected_empty)
					{
						selected_y = i, selected_x = j;
						selected_injury = injury, selected_empty = empty;
					}
	...
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
		{
			...
			{
				int ans = 1;
				for (int score = 0; score < satisfaction - 1; score++) ans *= 10;
				answer += ans;
			}
		}

...
int dx[4] = { 0, -1, 0, 1 }; // 방향에 따른 x좌표 변화량
int dy[4] = { 1, 0, -1, 0 }; // 방향에 따른 y좌표 변화량
...
void check(int x, int y, int dir)
{
	...
	while (1)
	{
		...
		x = nx; // 다음 위치로 x좌표 갱신
		y = ny; // 다음 위치로 y좌표 갱신
		if (nx < 0 || ny < 0 || nx >= N || ny >= M) return; // 범위를 벗어난 경우 반환
		if (arr[nx][ny] == 6) return; // 벽인 경우 반환
		if (arr[nx][ny] != 0) continue; // 이미 CCTV가 있거나, 이미 감시되는 영역인 경우 넘어감
		arr[nx][ny] = -1; // CCTV로부터 감시되는 영역이므로 -1로 표시
	}
}

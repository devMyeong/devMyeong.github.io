...
int rows, columns, target; // 행, 열, 타겟값
...
void sortRows() // 각 행에 대해 정렬하는 함수
{
	...
	{
		int freq[102] = { 0 }; // 각 숫자의 등장 횟수를 저장하는 배열
		for (int j = 0; j < colCount; j++) // 현재 행의 모든 열에 대해 반복
		{
			freq[grid[i][j]]++; // 해당 숫자의 등장 횟수 증가
		}

		std::vector<std::pair<int, int>> freqPairs; // 등장 횟수와 해당 숫자를 저장하는 벡터
		for (int j = 1; j < 101; j++) // 1부터 100까지 모든 숫자에 대해 반복
		{
			if (freq[j] > 0) // 해당 숫자가 등장한 경우
			{
		...
		cnt = std::max(cnt, size * 2); // 열 개수를 갱신
...
void sortColumns()
{
	...
	for (int i = 0; i < colCount; i++)
	{
		...
		cnt = std::max(cnt, size * 2);  // 현재 가장 긴 열의 길이 업데이트
...
int main()
{
	...
	while (time <= 100)
	{
		if (grid[rows - 1][columns - 1] == target) break; // 목표값이 나오면 종료
		time++;
		if (rowCount >= colCount) sortRows(); // 행 기준으로 정렬
		else sortColumns(); // 열 기준으로 정렬
	}
...
int matrix[101][101], rows = 3, cols = 3; // 행렬, 행의 개수, 열의 개수
int target_row, target_col, target_value; // 찾아야 하는 값이 있는 위치의 행, 열, 값
int answer = 0; // 정답
...
int main()
{
	...
	cin >> target_row >> target_col >> target_value;
	--target_row;
	--target_col;
	...
	while (1) // 무한루프
	{
		if (matrix[target_row][target_col] == target_value) break; // 타겟 숫자를 찾았으면 루프 종료

		answer++; // 연산 횟수 증가
		if (answer > 100) // 100회 이상 연산해도 타겟 숫자를 찾지 못한 경우
		{
			answer = -1; // -1 반환하고 루프 종료
			break;
		}

		vector<pair<int, int>> freq[101]; // (숫자, 숫자의 빈도수) pair가 저장될 2차원 배열

		// 행 연산
		if (rows >= cols) // 행의 개수가 열의 개수 이상인 경우
		{
			for (int i = 0; i < rows; i++) // 각 행을 순회하면서
			{
				int cnt[101] = { 0, }; // 현재 행에서 각 열의 숫자가 나오는 빈도수 확인
				for (int j = 0; j < cols; j++)
				{
					if (matrix[i][j] > 0) cnt[matrix[i][j]]++;
				}

				for (int j = 1; j <= 100; j++) // 빈도수가 1 이상인 숫자를 freq 배열에 저장
				{
					if (cnt[j] > 0) freq[i].push_back({ cnt[j], j }); // 빈도수, 숫자
				}
			}
			...
			for (int i = 0; i < rows; i++)
			{
				for (int j = 0; j < cols; j++)
				{
					matrix[i][j] = 0; // 기존 matrix 배열을 0으로 초기화
				}
			}
			...
			for (int i = 0; i < rows; i++)
			{
				sort(freq[i].begin(), freq[i].end()); // 각 행의 빈도수 순으로 오름차순 정렬
			}

			// 2차원 배열의 내용을 matrix에 넣어 배열의 형태 변경
			for (int i = 0; i < rows; i++)
			{
				int tmp = 0; // 갱신될 열의 크기
				for (int j = 0; j < freq[i].size(); j++)
				{
					matrix[i][tmp++] = freq[i][j].second; // 숫자
					matrix[i][tmp++] = freq[i][j].first; // 빈도수
				}
				cols = max(cols, tmp); // 열의 크기 갱신
			}
		}
		// 열의 개수가 행의 개수 이상인 경우 열연산
		else
		{
			for (int i = 0; i < cols; i++)
			{
				...
				{
					if (matrix[j][i] > 0) cnt[matrix[j][i]]++;
				}

				for (int j = 1; j <= 100; j++)
				{
					if (cnt[j] > 0) freq[i].push_back({ cnt[j], j });
				}
			}
			...
			for (int i = 0; i < cols; i++)
			{
				...
				for (int j = 0; j < freq[i].size(); j++)
				{
					matrix[tmp++][i] = freq[i][j].second;
					matrix[tmp++][i] = freq[i][j].first;
				}
				rows = max(rows, tmp);
			}
		}

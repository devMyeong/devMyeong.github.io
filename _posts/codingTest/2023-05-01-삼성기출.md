...
int main()
{
	...
	int directionX[9] = { 0, 0, -1, -1, -1, 0, 1, 1, 1 };
	int directionY[9] = { 0, -1, -1, 0, 1, 1, 1, 0, -1 };
	...
	while (numMoves--)
	{
		...
		{
			int x = cloudQueue.front().first;
			int y = cloudQueue.front().second;
			...
			grid[x][y]++;
			visited[x][y] = true;
			cloudPositions.push_back(make_pair(x, y));
		}

		// 현재 구름 위치에서 대각선 방향으로 인접한 칸의 물의 양 증가시키기
		for (int i = 0; i < cloudPositions.size(); i++)
		{
			int x = cloudPositions[i].first;
			int y = cloudPositions[i].second;
			...
			for (int j = 2; j <= 8; j += 2)
			{
				...
				if (1 <= nx && nx <= gridSize && 1 <= ny && ny <= gridSize && 1 <= grid[nx][ny])
					grid[x][y]++;
			}
		}

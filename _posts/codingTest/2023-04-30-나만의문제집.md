...
{
	if (r == rows) // 모든 행에 대한 숫자 조합이 결정된 경우
	{
		int sum = 0; // 숫자 조합의 합
		for (int r = 0; r < rows; r++) // 각 행에 대해
		{
			int temp = 0; // 현재 숫자 조합
			for (int c = 0; c < cols; c++)
			{
				if (visited[r][c]) // 방문한 열의 숫자를 숫자 조합에 추가
					temp = temp * 10 + board[r][c];
				else // 방문하지 않은 열을 만난 경우
				{
		...
		for (int c = 0; c < cols; c++) // 각 열에 대해
		{
			int temp = 0; // 현재 숫자 조합
			for (int r = 0; r < rows; r++)
			{
				if (!visited[r][c]) // 방문하지 않은 행의 숫자를 숫자 조합에 추가
					temp = temp * 10 + board[r][c];
				else // 방문한 행을 만난 경우
				{
					sum += temp; // 숫자 조합의 합에 현재 숫자 조합을 더함
	...
	if (c == cols) // 한 행에 대한 모든 열에 대한 숫자 조합이 결정된 경우
	{
		dfs(r + 1, 0); // 다음 행을 탐색
		return;
	}

	visited[r][c] = true; // 현재 열의 숫자를 방문한 경우
	dfs(r, c + 1); // 다음 열을 탐색

	visited[r][c] = false; // 현재 열의 숫자를 방문하지 않은 경우
	dfs(r, c + 1);
}

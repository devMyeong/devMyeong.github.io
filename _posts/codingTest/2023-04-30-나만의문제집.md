...
{
	if (k == emptyCellCount) return 1; // 모든 빈 셀에 대한 수를 다 채웠을 경우 1을 반환

	int x = emptyCells[k][0], y = emptyCells[k][1]; // 현재 채워야 할 빈 셀의 좌표
	int startX = x / 3 * 3, startY = y / 3 * 3; // 현재 채워야 할 빈 셀이 속한 3x3 영역의 시작 좌표
	int check[10] = { 0 }; // 현재 채워야 할 빈 셀에서 가능한 숫자를 체크하는 배열
	int result = 0; // 백트래킹 결과를 저장하는 변수
	...
	for (int i = 1; i <= 9; i++)
	{
		if (check[i] == 0) // 숫자 i가 사용되지 않았을 경우
		{
			sudoku[x][y] = i; // 숫자 i를 현재 채워야 할 빈 셀에 대입
			result = backtracking(k + 1); // 다음 빈 셀로 넘어가기 위해 백트래킹 수행
			if (result == 1) break; // 백트래킹 결과가 1인 경우 (스도쿠가 완성된 경우) 루프 종료
			sudoku[x][y] = 0; // 백트래킹 결과가 0인 경우 (스도쿠가 완성되지 않은 경우) 숫자 i를 제거하고 다른 숫자 시도
		}
	}

...
### 03-7 플로이드 ( 11404 )
- 모든 도시의 쌍 (A, B)에 대해서 도시 A에서 B로 가는데 필요한 비용의 최솟값을 구하는 프로그램을 작성하시오 ([**참고**](https://www.acmicpc.net/problem/11404))

```cpp
```

### 03-8 운동 ( 1956 )
- 도로의 정보가 주어졌을 때, 도로의 길이의 합이 가장 작은 사이클을 찾는 프로그램을 작성하시오 두 마을을 왕복하는 경우도 사이클에 포함됨에 주의한다 ([**참고**](https://www.acmicpc.net/problem/1956))
...
int main()
{
    // knots 벡터의 크기는 n + k + 1이어야 합니다 ( Clamped at start, Clamped at end )
